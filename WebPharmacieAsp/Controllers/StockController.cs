...
namespace WebPharmacieAsp.Controllers
{
    public class StockController : Controller
    {
        // GET: Stock
        [HttpGet]
        public async Task<ActionResult> ListeStock()
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                var response = await client.GetAsync
                    (
                  "http://localhost:81/PharmacieWeb/api/Stock"
                    );
        ...
        public async Task<ActionResult> Edit(int id)
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                var response = await client.GetAsync
                    (
                    "http://localhost:81/PharmacieWeb/api/Stock?id=" + id
                    );
                if (response.IsSuccessStatusCode)
                {
                    var json = await response.Content.ReadAsStringAsync();
        ...
        public async Task<ActionResult> Edit(Categorie model)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    return RedirectToAction("ListeStock");
                }
            }
            catch (Exception ex)
            {
